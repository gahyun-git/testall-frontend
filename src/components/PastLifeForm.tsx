...
export default function PastLifeForm() {
  ...
  // 상대경로로 구성된 API 엔드포인트
  const apiUrl = `${API_BASE}/past-life/take_test/`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.result_id) {
        console.log('Test started successfully:', data);
        router.push(`/past-life/result/${data.result_id}`);
      } else {
        ...
        console.log('Response Result ID:', data.result_id);
        console.log('Response Error:', data.error);
        console.log('Response Message:', data.message);
        alert('테스트 진행 중 오류가 발생했습니다.');
      }
    } catch (error) {
      console.error(error);
